...
public final class SQLiteManifestStore: ManifestStore {
    ...
    public func filesToVerify(before date: Date, limit: Int) throws -> [FileRecord] {
    ...
    private func prepareStatements() throws {
        ...
        stmtSelectFile = try prepare("SELECT id, path, size, mtime, hash, hash_algorithm, first_seen, last_verified, last_modified, status FROM files WHERE path = ?")

        stmtAllPaths = try prepare("SELECT path FROM files")

        stmtMarkMissing = try prepare("UPDATE files SET status = 'missing' WHERE path = ?")

        stmtInsertEvent = try prepare("INSERT INTO events (timestamp, event_type, path, detail) VALUES (?, ?, ?, ?)")
    ...
    private func bindAndStepUpsert(_ record: FileRecord) throws {
        ...
        bindOptionalDouble(stmt, index: 8, value: record.lastModified?.timeIntervalSince1970)
