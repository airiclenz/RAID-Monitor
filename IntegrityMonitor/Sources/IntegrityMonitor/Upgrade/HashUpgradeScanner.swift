...
public struct HashUpgradeScanner {
    ...
    public init(store: ManifestStore, alertManager: AlertManager, logger: Logger) {
        self.store = store
    ...
    public func upgrade(from oldAlgorithm: String, to newAlgorithm: String) async throws -> UpgradeResult {
        ...
        for record in candidates {
            ...
            upgraded.hashAlgorithm = newAlgorithm
            upgraded.lastVerified = Date()
            upgraded.status = .ok
            batch.append(upgraded)
            result.upgraded += 1
