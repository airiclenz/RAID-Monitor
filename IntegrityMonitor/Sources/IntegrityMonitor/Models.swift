...
public struct RAIDArrayInfo: Equatable {
    ...
    public init(uuid: String, name: String, status: String, members: [RAIDMemberInfo]) {
        self.uuid = uuid
        self.name = name
        self.status = status
...
public struct RAIDMemberInfo: Equatable {
    ...
    public init(devNode: String, status: String, smartStatus: SMARTStatus = .unknown) {
...
public struct Alert {
    ...
    public init(title: String, subtitle: String = "", body: String, severity: AlertSeverity, timestamp: Date = Date()) {
        self.title = title
...
public enum AppError: Error, CustomStringConvertible {
    ...
    public var description: String {
        ...
        case .processFailure(let cmd, let code, let stderr):
            return "Process '\(cmd)' failed with exit code \(code): \(stderr)"
        case .fileAccess(let path, let err):
            return "Cannot access \(path): \(err.localizedDescription)"
        }
    }
}
