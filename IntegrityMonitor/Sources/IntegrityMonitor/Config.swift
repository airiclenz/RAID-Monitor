...
public struct Config: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        watchPaths            = try c.decode([String].self, forKey: .watchPaths)
        exclude               = try c.decodeIfPresent(ExclusionConfig.self,    forKey: .exclude)               ?? ExclusionConfig()
        hashAlgorithm         = try c.decodeIfPresent(String.self,             forKey: .hashAlgorithm)         ?? "sha256"
        verificationIntervalDays = try c.decodeIfPresent(Int.self,             forKey: .verificationIntervalDays) ?? 30
        database              = try c.decodeIfPresent(DatabaseConfig.self,     forKey: .database)              ?? DatabaseConfig()
        notifications         = try c.decodeIfPresent(NotificationConfig.self, forKey: .notifications)         ?? NotificationConfig()
        ...
        raid                  = try c.decodeIfPresent(RAIDConfig.self,         forKey: .raid)                  ?? RAIDConfig()
        schedule              = try c.decodeIfPresent(ScheduleConfig.self,     forKey: .schedule)              ?? ScheduleConfig()
    }
}
...
public struct ExclusionConfig: Codable {
    ...
    public init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        pathPatterns      = try c.decodeIfPresent([String].self, forKey: .pathPatterns)      ?? []
        directoryPatterns = try c.decodeIfPresent([String].self, forKey: .directoryPatterns) ?? []
        minSizeBytes      = try c.decodeIfPresent(Int.self,      forKey: .minSizeBytes)
        maxSizeBytes      = try c.decodeIfPresent(Int.self,      forKey: .maxSizeBytes)
    }
}
...
public struct NotificationConfig: Codable {
    ...
    public init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        onCorruption             = try c.decodeIfPresent(Bool.self, forKey: .onCorruption)             ?? true
        onRAIDDegraded           = try c.decodeIfPresent(Bool.self, forKey: .onRAIDDegraded)           ?? true
        onMissingFile            = try c.decodeIfPresent(Bool.self, forKey: .onMissingFile)            ?? false
        onScanComplete           = try c.decodeIfPresent(Bool.self, forKey: .onScanComplete)           ?? false
        onScanCompleteWithIssues = try c.decodeIfPresent(Bool.self, forKey: .onScanCompleteWithIssues) ?? true
    }
}
...
public struct PerformanceConfig: Codable {
    ...
    public init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        maxHashThreads        = try c.decodeIfPresent(Int.self, forKey: .maxHashThreads)        ?? 2
        dbBatchSize           = try c.decodeIfPresent(Int.self, forKey: .dbBatchSize)           ?? 500
        maxVerificationsPerRun = try c.decodeIfPresent(Int.self, forKey: .maxVerificationsPerRun) ?? 1000
    }
}
...
public struct LoggingConfig: Codable {
    ...
    public init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        logPath         = try c.decodeIfPresent(String.self, forKey: .logPath)         ?? "~/.local/share/raid-integrity-monitor/raid-integrity-monitor.log"
        level           = try c.decodeIfPresent(String.self, forKey: .level)           ?? "info"
        maxLogSizeBytes = try c.decodeIfPresent(Int.self,    forKey: .maxLogSizeBytes) ?? (10 * 1024 * 1024)
    }
}
...
public struct ScheduleConfig: Codable {
    ...
    public init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        raidCheckIntervalMinutes = try c.decodeIfPresent(Int.self, forKey: .raidCheckIntervalMinutes) ?? 5
        fileScanIntervalHours    = try c.decodeIfPresent(Int.self, forKey: .fileScanIntervalHours)    ?? 24
    }
}
...
public struct RAIDConfig: Codable {
    ...
    public init(enabled: Bool = true, memberDisks: [String] = []) {
    ...
    public init(from decoder: Decoder) throws {
        ...
        memberDisks = try c.decodeIfPresent([String].self, forKey: .memberDisks) ?? []
    }
}
