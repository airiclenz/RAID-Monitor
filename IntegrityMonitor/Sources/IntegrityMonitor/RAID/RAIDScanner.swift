...
public struct RAIDScanner {
    ...
    public init(config: RAIDConfig, logger: Logger) {
        self.config = config
        self.logger = logger
    }
            ...
            arrays = arrays.map { array in
                ...
                updated.members = array.members.map { member in
                    ...
                    if config.memberDisks.contains(parentDisk) || config.memberDisks.contains(member.devNode) {
                        m.smartStatus = checkSMART(disk: parentDisk)
                        logger.info("SMART: /dev/\(parentDisk) (\(member.devNode)): \(m.smartStatus.rawValue)")
                    }
                    return m
                }
                return updated
    ...
    private func runDiskutil() throws -> String {
        ...
        do {
            ...
            throw AppError.processFailure(command: "diskutil appleRAID list", exitCode: -1, stderr: error.localizedDescription)
        }
        ...
        if process.terminationStatus != 0 {
            throw AppError.processFailure(command: "diskutil appleRAID list",
...
struct RAIDOutputParser {
    ...
    private static func isRule(_ line: String, char: Character) -> Bool {
    ...
    private static func parseMemberLine(_ rawLine: String) -> RAIDMemberInfo? {
        ...
        return RAIDMemberInfo(devNode: devNode, status: status.isEmpty ? "Unknown" : status)
    }
